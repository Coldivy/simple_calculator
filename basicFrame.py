...
from calcu import Ui_Form
from ctypes import *

c_lib = CDLL('./cal_c.dll')

c_lib.calculate.restype = c_double

def python_calculate(expression):
    # 将Python字符串编码为C字符串
    c_expression = expression.encode('utf-8')
    # 调用C函数
    result = c_lib.calculate(c_expression)
    return result
...
class MyWindows(QWidget,Ui_Form):
    ...
    def equal(self):
        self.result = str(python_calculate(self.result))
        self.lineEdit.setText(str(self.result))
